...
async function createProgramFromGeneratedPlan(params: {
  ...
  const { aiPlan, clinicianId, isTemplate, aiGenerationParams, patientId, startDate } = params;
  ...
  const program = await prisma.program.create({
    data: {
      ...
      isTemplate: !patientId && isTemplate,
...
export async function generateProgramAction(
  ...
  try {
    ...
    revalidatePath("/programs");
    if (params.patientId) revalidatePath(`/patients/${params.patientId}`);
...
export async function saveGeneratedProgramAction(input: {
  ...
  try {
    const program = await createProgramFromGeneratedPlan({
      aiPlan: input.aiPlan,
      clinicianId: user.id,
      isTemplate: !input.patientId && input.isTemplate,
      aiGenerationParams: input.params,
      patientId: input.patientId || null,
      startDate: input.patientId
        ? (input.startDate ?? new Date().toISOString().split("T")[0])
        : null,
    });

    revalidatePath("/programs");
    if (input.patientId) revalidatePath(`/patients/${input.patientId}`);
    return { success: true as const, data: program.id };
  } catch (error) {
